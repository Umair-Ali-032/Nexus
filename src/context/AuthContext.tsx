...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const storedUser = localStorage.getItem(USER_STORAGE_KEY);
    if (storedUser) {
      setUser(JSON.parse(storedUser) as Entrepreneur | Investor);
    }
    setIsLoading(false);
  }, []);
  ...
  const login = async (email: string, _password: string, role: UserRole): Promise<void> => {
    ...
    try {
      ...
      if (foundUser) {
        setUser(foundUser as Entrepreneur | Investor); // ✅ Type assertion
        localStorage.setItem(USER_STORAGE_KEY, JSON.stringify(foundUser));
        toast.success('Successfully logged in!');
      } else {
        throw new Error('Invalid credentials or user not found');
    ...
    try {
      ...
      if (role === 'entrepreneur') {
        ...
        newUser = {
          ...
          portfolioCompanies: [],
          totalInvestments: 0 ,
          minimumInvestment:" 1000 ",  // <-- Required
          maximumInvestment: "100000"  // <-- Required
      };
        
      }

      users.push(newUser);
      setUser(newUser as Entrepreneur | Investor); // ✅ Type assertion
      localStorage.setItem(USER_STORAGE_KEY, JSON.stringify(newUser));
      toast.success('Account created successfully!');
    } catch (error) {
      toast.error((error as Error).message);
